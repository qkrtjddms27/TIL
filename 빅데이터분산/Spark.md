...
- 스파크 기반의 SQL이 가능해짐
- 자바의 ArrayList 또는 스칼라의 Seq 객체 등의 고정 타입형 컬렉션.
- 대규모 애플리케이션을 개발하는 데 특히 유용.

#### Dataset
- Dataset은 스칼라의 정적 데이터 타입에 맞는 코드, 정적 타입 코드를 지원하기 위해 고안된 스파크의 구조적 API.
- 파이썬과 R에서는 사용될 수 없다.
...
#### Driver
- 스파크 애플리케이션의 실행을 제어하고 스파크 클러스터의 모든 상태 정보를 유지.
- 물리적 컴퓨팅 자원 확보와 익스큐터 실행을 위해 클러스터 매니저와 통신할 수 있어야 합니다.
- 한 개의 노드에서 실행되며, 스파크 전체의 main() 함수를 실행.
- 어플리케이션 내 정보의 유지 관리, 익스큐터의 실행 및 실행 분석, 배포 등의 역할.

#### Executor
- Spark Driver가 할당한 작업을 수행하여 결과를 반환.
- 태스크의 상태와 결과를 드라이버에 보고.

#### 클러스터 매니저
- 클러스터 매니저는 스파크 애플리케이션을 실행할 클러스터 머신을 유지.
- 물리적인 머신에 연결되는 개념.
- 스파크가 지원하는 클러스터 매니저는 스탠드얼론 클러스터 매니저, 아파치 메소스, 하둡 YARN

### 운영용 애플리케이션 실행하기
- spark-submit 명령을 사용하면 대화형 셀에서 개발한 프로그램을 운영용 애플리케이션으로 쉽게 전환 가능.
- spark-submit 명령은 애플리케이션 코드를 클러스터에 전송해 실행시키는 역활을 수행.
- 클러스터에 제출된 애플리케이션은 작업이 종료되거나 에러가 발생할 때까지 실행.


### 실행 모드
애플리케이션을 실행할 때 요청한 자원의 물리적인 위치를 결정.
- 클러스터 모드
- 클라이언트 모드
- 로컬 모드
...
- 클러스터 매니저는 파일을 받은 다음 워커 노드에 드라이버와 익스큐터 프로세스를 실행.
- 하나의 워커 노드에 스파크 드라이버를 할당하고, 다른 워커 노드에 익스큐터를 할당하는 모습.

#### 클라이언트모드
- 애플리케이션을 제출한 클라이언트 머신에 스파크 드라이버가 위치하는 것을 제외하면 클러스터 모드와 비슷.
- 애플리케이션이 클러스터와 무관한 머신에서 동작.
- 게이트웨이 머신, 애지 노드라고 부름.

#### 로컬모드
- 모든 스파크 애플리케이션은 단일 머신에서 실행.
- 애플리케이션의 병렬 처리를 위해 단일 머신의 스레드를 활용.
- 애플리케이션 테스트 그리고 개발 중인 애플리케이션을 반복적으로 실험하는 용도로 주로 사용.


### 스파크 애플리케이션의 생애주기 (스파크 외부)
스파크 애플리케이션은 컴파일된 JAR나 라이브러리 파일을 의미.

#### 클라이언트 요청
- 스파크 애플리케이션을 제출하는 시점에 로컬 머신에서 코드가 실행되어 클러스터 드라이버 노드에 요청.
- 스파크 드라이버 프로세스의 자원을 함께 요청.
- 클러스터 매니저는 이 요청을 받아들이고 클러스터 

#### 시작
- 드라이버 프로세스가 클러스터에 배치되었으므로 사용자 코드를 실행할 차례.
- 사용자 코드에는 반드시 스파크 클러스터를 초기화하는 SparkSession이 포함.
  + SparkSession은 클러스터 매니저와 통신해 스파크 익스큐터 프로세스의 실행을 요청.
- 사용자는 spark-submit을 실행할때 사용하는 명령행 인수로 익스큐터 수와 설정값을 지정.

#### 실행
- 드라이버와 워커는 코드를 실행하고 데이터를 이동하는 과정에서 서로 통신.
- 드라이버는 각 워커에 태스크를 할당.
- 태스크를 할당받은 워커는 태스크 상태와 성공/실패 여부를 드라이버에 전송.

#### 완료
- 스파크 애플리케이션의 실행이 완료되면 드라이버 프로세스가 성공이나 실패 중 하나의 상태로 종료.
- 클러스터 매니저는 드라이버가 속한 스파크 클러스터의 모든 익스큐터를 종료.
- 스파크 애플리케이션의 성공/실패 여부를 클러스터 매니저에 요청해 확인할 수 있다.

### 스파크 애플리케이션의 생애주기 (스파크 내부)
- 스파크 애플리케이션은 하나 이상의 스파크 잡으로 구성.
- 스레드를 사용해 여러 액션을 병렬로 수행하는 경우가 아니라면 애플리케이션의 스파크 잡은 차례대로 실행.

#### SparkSession
- 모든 스파크 애플리케이션은 가장 먼저 SparkSession을 생성.
- 대화형 모드에서는 자동을 생성되지만, 애플리케이션을 만드는 경우라면 직접 생성해야 한다.
- new SparkContext 패턴 보다는 SparkSession의 빌더 메서드를 활용하여 생성하는 것이 좋다.
```scala
